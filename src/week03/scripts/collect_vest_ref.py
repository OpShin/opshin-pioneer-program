...
def main(name: str, parameterized):
    ...
    plutus_script_ref_utxo = get_ref_utxo(plutus_script, context)
    ...
    for utxo in context.utxos(script_address):
        if utxo.output.datum:
            if parameterized:
                ...
                if isinstance(utxo.output.datum, RawCBOR):
                    try:
    ...
    builder.add_script_input(
        utxo_to_spend, script=plutus_script_ref_utxo or plutus_script, redeemer=redeemer
    )
